...
from .. import db
from ..utils import categorie_to_dict_1, get_all_categorie, get_categorie, create_categorie, \
    update_categorie
...
def create_or_get_all_categories():
    if request.method == "GET":
        try:
            ...
            return jsonify([categorie_to_dict_1(cat) for cat in categories]), 200
...
def get_or_update_or_delete_categorie(numCat):
    try:
        ...
        if not categorie:
            ...
            if request.method == "GET":
                return jsonify(categorie_to_dict_1(categorie))
